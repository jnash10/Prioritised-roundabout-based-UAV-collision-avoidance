    ...
    def avoid_collision(self, drones): 
        ...
        for drone in drones:
            if drone.name != self.name:
                if np.linalg.norm((drone.x-self.x,drone.y-self.y))<3:
                    vec += drone.v*np.array([drone.x-self.x,drone.y-self.y])/(np.linalg.norm((drone.x-self.x,drone.y-self.y)))**2
                    #vec += drone.v*np.array([drone.x-self.x,drone.y-self.y])
