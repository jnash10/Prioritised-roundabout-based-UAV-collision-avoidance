    ...
    def avoid_collision(self, drones): 
        ...
        for drone in drones:
            if drone.name != self.name:
                if np.linalg.norm((drone.x-self.x,drone.y-self.y))<maneuver_distance:
                    if self.zem(drone) < safe_distance and self.t_go(drone)>=0:
                        ...
                        tangential_vec = np.array([np.sin(angle),-np.cos(angle)])
                        #vec += self.v*(tangential_vec + curvec)/(self.priority/drone.priority)**2
                        vec += self.v*((drone.priority/self.priority)**4)*(tangential_vec + curvec)
        return vec
