    ...
    def avoid_collision(self, drones): 
        ...
        for drone in drones:
            if drone.name != self.name:
                if np.linalg.norm((drone.x-self.x,drone.y-self.y))<7:
                    angle = np.arctan2(drone.y-self.y,drone.x-self.x) 
                    curvec = ((drone.priority)**2)*np.array([np.cos(angle),np.sin(angle)])/np.linalg.norm((drone.x-self.x,drone.y-self.y))**2
                    tangential_vec = ((drone.priority)**2)*np.array([np.sin(angle),-np.cos(angle)])
                    vec += self.v*(tangential_vec + curvec)/(self.priority)**1
        return vec
