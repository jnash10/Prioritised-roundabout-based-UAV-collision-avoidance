            ...
            if drone.name != self.name:
                if np.linalg.norm((drone.x-self.x,drone.y-self.y))<detection_distance:
                    if self.zem(drone) < safe_distance and self.t_go(drone)>=0:
                        ...
                        # curvec = np.array([np.cos(angle),np.sin(angle)])/np.linalg.norm((drone.x-self.x,drone.y-self.y))**2
                        # #curvec = np.array([0,0])
                        # tangential_vec = np.array([np.sin(angle),-np.cos(angle)])/np.linalg.norm((drone.x-self.x,drone.y-self.y))**2
                        # #vec += self.v*(tangential_vec + curvec)/(self.priority/drone.priority)**2
                        # vec += self.v*((drone.priority/self.priority)**2)*(tangential_vec + curvec)
                        dist = np.linalg.norm((drone.x-self.x,drone.y-self.y))
                        f_v = (k/dist**3)*(1/dist - 1/detection_distance)*np.array([drone.x-self.x, drone.y-self.y])
                        del_v = ((drone.priority/self.priority)**2)*f_v
                        #print(self.name, del_v)
                        #vec = vec  + self.v*del_v*np.array([np.cos(angle+np.pi/2), np.sin(angle+np.pi/2)])
                        vec = vec  + self.v*del_v
                        #+ self.v*del_v*np.array([np.cos(angle), np.sin(angle)])
        return vec
