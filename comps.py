...
class Uav:
            ...
            if drone.name != self.name:
                if np.linalg.norm((drone.x-self.x,drone.y-self.y))<detection_distance:
                    if self.zem(drone) < safe_distance and self.t_go(drone)>=0:
                        ...
                        f_v = (k/(dist-safe_distance)**3)*(1/(dist-(drone.priority/self.priority)*safe_distance) - 1/detection_distance)*np.array([drone.x-self.x, drone.y-self.y])
                        theta = np.pi/2
    ...
    def check_end(self, drones):
        if (np.linalg.norm(np.array([self.goal[1]-self.y,self.goal[0]-self.x]))<=0.2) and self.check==True:
...
def create_points():
    ...
    for i in range(n):
        ...
        goals.append((radius*-np.cos(i*step), radius*-np.sin(i*step)))
    return goals
...
def create_drones(goals):
    ...
    for i in range(len(goals)):
        #uav = Uav(f'uav{i+1}', 5, np.random.normal(loc=(3.5), scale=1.2), goals[i])
        uav = Uav(f'uav{i+1}', 1, 1, goals[i])
        #uav = Uav(f'uav{i+1}', 5, (i%5)+1, goals[i])
